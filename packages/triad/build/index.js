 ...
 */
function fromScale(scale, steps = 2) {}
 ...
 */
function intervals(st) {
  var inner = st.split("").map(t => (t === "m" ? "3m" : "3M"));
  return inner.reduce(
    function(ivls, v) {
      ivls.push(tonalTranspose.transpose(v, ivls[ivls.length - 1]));
      return ivls;
    },
    ["1P"]
  );
...
function allFor(st) {
  return sortedSet(tonalArray.permutations(st.split("")).map(s => s.join("")));
}
